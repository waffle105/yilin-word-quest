...
async function userId() {
  ...
  return h.get("oai-authenticated-user-id") ?? "preview-user";
}

export async function GET() {
  try {
    const uid = await userId();
    const rows = await env.DB.prepare(`SELECT word, attempts, correct_count AS correctCount,
      ...
      FROM word_progress WHERE user_id = ? ORDER BY last_seen_at DESC`).bind(uid).all();
...
export async function POST(request: Request) {
  try {
    ...
    const now = new Date();
    const existing = await env.DB.prepare(`SELECT attempts, correct_count AS correctCount,
      spelling_errors AS spellingErrors, reading_errors AS readingErrors, mastery,
      interval_days AS intervalDays FROM word_progress WHERE user_id = ? AND word = ?`)
      .bind(uid, word).first<Record<string, number>>();
    ...
    const upsert = env.DB.prepare(`INSERT INTO word_progress
      ...
      .bind(uid, word, next.attempts, next.correctCount, next.spellingErrors, next.readingErrors,
        mastery, intervalDays, due.toISOString(), now.toISOString());
    const log = env.DB.prepare(`INSERT INTO practice_attempts
      (user_id, word, spelling_correct, reading_correct, response_ms, created_at) VALUES (?, ?, ?, ?, ?, ?)`)
      .bind(uid, word, body.spellingCorrect ? 1 : 0, body.readingCorrect ? 1 : 0,
        Math.max(0, Math.min(300000, Number(body.responseMs ?? 0))), now.toISOString());
    await env.DB.batch([upsert, log]);
